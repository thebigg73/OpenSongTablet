...
import android.widget.TextView; // Use native TextView instead of AppCompatTextView
...
public class MyMaterialSimpleTextView extends TextView {
    ...
    public void setPalette(@NonNull Palette palette) {
        ...
        if (xmlDrawableResId != 0) {
            Drawable drawable = ContextCompat.getDrawable(getContext(), xmlDrawableResId);
            if (drawable != null) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            ...
            // Fallback for pre-M devices where compound drawable tint list isn't natively supported
            Drawable[] currentDrawables = getCompoundDrawablesRelative();
            for (Drawable drawable : currentDrawables) {
                if (drawable != null) {
                    Drawable wrapped = DrawableCompat.wrap(drawable.mutate());
                    DrawableCompat.setTint(wrapped, palette.textColor);
                }
            }
        }
    }
