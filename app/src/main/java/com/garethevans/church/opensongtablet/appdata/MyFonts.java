...
public class MyFonts {
    ...
    private ArrayList<String> monoFontNames = new ArrayList<>();
    ...
    public void setMonoFont(String monoFontName, Typeface monoFont) {
        this.monoFontName = monoFontName;
    ...
    public void setUpAppFonts(Handler lyricFontHandler,
                              ...
                              Handler presoFontHandler, Handler presoInfoFontHandler, Handler monoFontHandler) {
        ...
        monoFontName = mainActivityInterface.getPreferences().getMyPreferenceString("fontMono", "Inconsolata");
        ...
        changeFont("fontMono",monoFontName,monoFontHandler);

        //setMonoFont(loadFontSafely("font/Inconsolata.ttf"));
    }
    ...
    public void changeFont(String which, String fontName, Handler handler) {
        ...
        if (fontName.startsWith("Fonts/")) {
            ...
            // Use the bundled lato font unless it is the mono font
            Typeface typeface;
            if (which.equals("fontMono")) {
                typeface = loadFontSafely("font/Inconsolata.ttf");
            } else {
                typeface = loadFontSafely("font/Lato.ttf");
            }
            doSetDesiredFont(which, typeface, fontName, null);
    ...
    public ArrayList<String> getFontsFromGoogle() {
        ...
        monoFontNames = new ArrayList<>(); // Separate list for Monospaced fonts
        String response = null;
        ...
        if (response == null || !hasPlayServices()) {
            // Fallback bundled fonts
            fontNames = bundledFonts();
            // Optional: If any bundled fonts are known monospace, you can add them to monoFontNames here as well
        } else {
            try {
                ...
                for (int i = 0; i < items.length(); i++) {
                    ...
                    String family = item.optString("family");
                    String category = item.optString("category");

                    if (family != null && !family.isEmpty()) {
                        String formattedFamilyForCheck = family.replace(" ", "_");

                        if (!notworking.contains(formattedFamilyForCheck + " ") &&
                                !family.startsWith("Noto Sans") &&
                                !family.startsWith("Noto Serif") &&
                                !family.startsWith("Rubik ")) {

                            // Add to general font list
                            fontNames.add(family);

                            // Populate the separate monospace arraylist with strict validation
                            if ("monospace".equalsIgnoreCase(category)) {
                                if (isTrustedMonospaceFont(family)) {
                                    monoFontNames.add(family);
                                }
                            }
                        }
                    }
                ...
                Log.e(TAG, "Error parsing Google Fonts JSON", e);
            }
        }

        // Add user storage fonts if any
        ArrayList<String> userFonts = mainActivityInterface.getStorageAccess().listFilesInFolder("Fonts","");
        if (userFonts != null && !userFonts.isEmpty()) {
            Collections.sort(userFonts, Collections.reverseOrder());
            for (String userFont : userFonts) {
        ...
        // You can return fontNames or monoFontNames depending on your needs.
        // To return the main list while keeping monoFontNames populated, return fontNames.
        return fontNames;
    }

    private boolean isTrustedMonospaceFont(String family) {
        if (family == null) return false;

        // Curated list of genuine, high-quality monospaced fonts available on Google Fonts
        List<String> trustedMonoList = Arrays.asList(
                "Roboto Mono",
                "Fira Code",
                "JetBrains Mono",
                "Source Code Pro",
        ...
        // Check if the family exists in your trusted list (case-insensitive search is safer)
        for (String trusted : trustedMonoList) {
            if (trusted.equalsIgnoreCase(family)) {
                return true;
            }
        }
        return false;
    ...
    public ArrayList<String> getMonoFontsFromGoogle() {
