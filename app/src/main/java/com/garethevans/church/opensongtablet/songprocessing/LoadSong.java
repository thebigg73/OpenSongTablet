...
public class LoadSong {
    ...
    public void doLoadSong(Context c, StorageAccess storageAccess, Preferences preferences, SongXML songXML, ProcessSong processSong,
                           SQLiteHelper sqLiteHelper, ConvertOnSong convertOnSong, ConvertChoPro convertChoPro) {
        ...
        if (filetype.equals("SONG") && !StaticVariables.songfilename.equals("Welcome to OpenSongApp")) {
            ...
            try {
                readAsXML = readFileAsXML(c, storageAccess, preferences, processSong);
        ...
        if (!readAsXML && StaticVariables.songfilename.toLowerCase(Locale.ROOT).contains(".onsong")) {
            ...
            try {
                readFileAsXML(c,storageAccess,preferences,processSong);
            } catch (Exception e) {
    ...
    private boolean readFileAsXML(Context c, StorageAccess storageAccess, Preferences preferences, ProcessSong processSong) throws XmlPullParserException, IOException {
                    ...
                    switch (xpp.getName()) {
                        case "author":
                            try {
                                ...
                                StaticVariables.mAuthor = fixXML(c, preferences, storageAccess,"author");
                            }
                            break;
                        case "copyright":
                        ...
                        case "lyrics":
                            try {
                                ...
                                // Try to read in the xml
                                e.printStackTrace();
                                StaticVariables.mLyrics = fixXML(c, preferences, storageAccess,"lyrics");
                            }
                            break;
                        case "ccli":
    ...
    private String fixXML(Context c, Preferences preferences, StorageAccess storageAccess, String section) {
