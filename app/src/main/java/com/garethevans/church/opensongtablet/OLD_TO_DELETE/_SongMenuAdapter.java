...
import com.garethevans.church.opensongtablet.PopUpCCLIFragment;
...
public class _SongMenuAdapter extends BaseAdapter implements SectionIndexer {
    ...
    private final ArrayList<_SongMenuViewItems> songList;
    private final _Preferences preferences;
    private ViewHolder viewHolder;
    private float textSize = 14.0f;

    @SuppressLint("UseSparseArrays")
    _SongMenuAdapter(Context context, _Preferences p, ArrayList<_SongMenuViewItems> songList) {
    ...
    public View getView(final int position , View convertView , ViewGroup parent ) {
        ...
        try {
            ...
            if (mInflater != null) {
                ...
                final _SongMenuViewItems song = songList.get(position);
                ...
                if (!isdirectory) {
                    viewHolder.lblListItem.setOnClickListener(_SongMenuListeners.itemShortClickListener(item_filename,item_key,position));
                    viewHolder.lblListItemAuthor.setOnClickListener(_SongMenuListeners.itemShortClickListener(item_filename,item_key,position));
                    viewHolder.lblListItem.setOnLongClickListener(_SongMenuListeners.itemLongClickListener(item_filename, StaticVariables.whichSongFolder, position));
                    viewHolder.lblListItemAuthor.setOnLongClickListener(_SongMenuListeners.itemLongClickListener(item_filename, StaticVariables.whichSongFolder, position));
                } else {
                            ...
                            if (songList.get(position).getInSet()) {
                                ...
                                if (userShouldConvertSongFormat(item_filename)) {
                                    ...
                                    if (preferences.getMyPreferenceBoolean(c,"ccliAutomaticLogging",false)) {
                                        ...
                                        String[] vals = _LoadXML.getCCLILogInfo(c, preferences, StaticVariables.whichSongFolder, item_filename);
    ...
    Map<String,Integer> getAlphaIndex(Context c, ArrayList<_SongMenuViewItems> songlist) {
