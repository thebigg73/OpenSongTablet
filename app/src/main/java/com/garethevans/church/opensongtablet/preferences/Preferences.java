...
public class Preferences extends Activity {
    ...
    // abcInlineWidth                   int         The number of pixels width for any inline abc notation (def:480)
    ...
    // beatBuddyControllerKitActive     boolean     When using the BeatBuddy controller, should the control be active (def:true)
    // beatBuddyControllerTempoActive   boolean     When using the BeatBuddy controller, should the control be active (def:true)
    // beatBuddyControllerVolActive     boolean     When using the BeatBuddy controller, should the control be active (def:false)
    // beatBuddyControllerVolHPActive   boolean     When using the BeatBuddy controller, should the control be active (def:false)
    ...
    // exportOpenSongSet                boolean     Should we export desktop xml file (def:false)
    ...
    // exportDesktop                    boolean     Should we export desktop xml file (def:false)
    // exportJustChords                 boolean     Should we export a song as .justchords (def:false)
    // exportSetSongs                   boolean     Should we include the set songs with the set export (def:false)
    ...
    // exportSetBundle                  boolean     Should we export an OpenSongApp set bundle (def:true)
    // exportSetBundleJustChords        boolean     Should we export a JustChords set bundle (def:false)
    ...
    // logIndexing                      boolean     Should we log the building of the song index (def:false)
    ...
    // metronomeVisualLength            int         Number of bars the visual metronome stays on for (0=indefinitely) (def:0)
    ...
    // nearbyBluetoothOnlyConnection    boolean     Should nearby connections stick to Bluetooth (useful for older devices)
    ...
    // songListFilterReset              boolean     Should we reset the song list filters when loading a song from a set/connected device (def:true)
    ...
    // voiceLiveChannel                 int         The MIDI channel for the VoiceLive 3 1-16 (def:1)
    // voiceLiveMajorHarmony            String      The default major harmony MAJ1, MAJ2, MAJ3 (def:MAJ1)
    // voiceLiveMinorHarmony            String      The default minor harmony MIN1, MIN2, MIN3 (def:MIN1)
