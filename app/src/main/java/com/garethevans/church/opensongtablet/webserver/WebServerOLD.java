...
public class WebServerOLD extends NanoHTTPD {
    ...
    public void callRunWebServer() {
        ...
        try {
            if (runWebServer) {
                ...
                mainActivityInterface.getKtorServer().start();
                // TODO remove once done
                runKtor();
            } else {
                //this.stop();
                mainActivityInterface.getKtorServer().stop();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getIP() {
        ...
        if (mainActivityInterface.getAppPermissions().hasWebServerPermission()) {
            ...
            if (ip.equals("http://0.0.0.0:8081/")) {
                try {
                    String wifiIp = null;
                        ...
                        while (enumInetAddress.hasMoreElements()) {
                            ...
                            if (inetAddress.getHostAddress() != null &&
                                ...
                                if (interfaceName.startsWith("wlan") || interfaceName.startsWith("eth")) {
                                    wifiIp = "http://" + ipAddr + ":8081/";
                                    break;
                                } else {
                                    // Save as fallback (e.g., tethering, VPN)
                                    if (fallbackIp == null) {
                                        fallbackIp = "http://" + ipAddr + ":8081/";
                                    }
                                }
                            }
                        }
