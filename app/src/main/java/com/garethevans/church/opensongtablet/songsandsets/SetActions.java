...
public class SetActions {
    ...
    void loadASet(Context c, Preferences preferences, StorageAccess storageAccess,
                  ProcessSong processSong) throws XmlPullParserException, IOException {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                if (xpp.getName().equals("slide_group")) {
                    ...
                    switch (xpp.getAttributeValue(null, "type")) {
                        ...
                        case "scripture":
                            // Get Scripture
                            getScripture(c, preferences, storageAccess, processSong);
                            break;
                        case "custom":
                            // Get Custom (Note or slide or variation)
                            getCustom(c, preferences, storageAccess, processSong);
                            break;
                        case "image":
                            // Get the Image(s)
                            getImage(c, preferences, storageAccess, processSong);
                            break;
                    }
                }
            }
    ...
    public void prepareSetList(Context c, Preferences preferences) {
        try {
            ...
            //TODO test set to remove
            setparse = "$**_MAIN/Abba Father_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father18_**$$**_Abba Father19_**$$**_Abba Father20_**$";
            setparse += "$**_MAIN/Abba Father_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father2_**$$**_Abba Father18_**$$**_Abba Father19_**$$**_Abba Father20_**$";
...
//		for (int x = FullscreenActivity.setSize-1; x<1; x--) {
    ...
    void saveSetMessage(Context c, Preferences preferences,
        ...
        if (StaticVariables.mSetList!=null && StaticVariables.mSetList.length>0) {
            ...
            if (!createNewSet.doCreation(c, preferences, storageAccess, processSong, loadSong,
                    songXML, convertOnSong, convertChoPro, sqLiteHelper)) {
    ...
    private void writeTempSlide(String where, String what, Context c, Preferences preferences, StorageAccess storageAccess, ProcessSong processSong) {
        ...
        my_NEW_XML += "<song>\n";
        my_NEW_XML += "  <title>" + processSong.parseToHTMLEntities(title) + "</title>\n";
        my_NEW_XML += "  <author>" + processSong.parseToHTMLEntities(author) + "</author>\n";
        my_NEW_XML += "  <user1>" + processSong.parseToHTMLEntities(user1) + "</user1>\n";
        my_NEW_XML += "  <user2>" + processSong.parseToHTMLEntities(user2) + "</user2>\n";
        my_NEW_XML += "  <user3>" + processSong.parseToHTMLEntities(user3) + "</user3>\n";
        my_NEW_XML += "  <aka>" + processSong.parseToHTMLEntities(aka) + "</aka>\n";
        my_NEW_XML += "  <key_line>" + processSong.parseToHTMLEntities(key_line) + "</key_line>\n";
        my_NEW_XML += "  <hymn_number>" + processSong.parseToHTMLEntities(hymn_number) + "</hymn_number>\n";
        my_NEW_XML += "  <lyrics>" + processSong.parseToHTMLEntities(lyrics) + "</lyrics>\n";
        my_NEW_XML += "</song>";
    ...
    private void getSong(ProcessSong processSong) {
        try {
            // Get path and remove leading /
            String p_name = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null,"path"));
            String s_name = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null,"name"));
    ...
    private void getScripture(Context c, Preferences preferences, StorageAccess storageAccess,
                              ProcessSong processSong) throws IOException, XmlPullParserException {
        ...
        while (!scripture_finished) {
            switch (xpp.getName()) {
                case "title":
                    ...
                    break;
                case "body":
                    scripture_text.append("\n[]\n").append(processSong.parseFromHTMLEntities(xpp.nextText()));
                    break;
                case "subtitle":
                    scripture_translation = processSong.parseFromHTMLEntities(xpp.nextText());
                    break;
            }
...
/*        for (String aTemp_text : temp_text) {
        ...
        writeTempSlide(c.getResources().getString(R.string.scripture), scripture_title, c,
                preferences, storageAccess, processSong);

        xpp.nextTag();
     }

    private void getCustom(Context c, Preferences preferences, StorageAccess storageAccess,
                           ProcessSong processSong) throws IOException, XmlPullParserException {
        ...
        String custom_name = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "name"));
        String custom_seconds = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "seconds"));
        String custom_loop = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "loop"));
        String custom_title = "";
        ...
        while (!custom_finished) {
            switch (xpp.getName()) {
                case "title":
                    custom_title = processSong.parseFromHTMLEntities(xpp.nextText());
                    break;
                case "notes":
                    custom_notes = processSong.parseFromHTMLEntities(xpp.nextText());
                    break;
                case "body":
                    custom_text.append("\n---\n").append(processSong.parseFromHTMLEntities(xpp.nextText()));
                    break;
                case "subtitle":
                    custom_subtitle = processSong.parseFromHTMLEntities(xpp.nextText());
                    break;
            }
        ...
        writeTempSlide(noteorslide, custom_name, c, preferences, storageAccess, processSong);

    }

    private void getImage(Context c, Preferences preferences, StorageAccess storageAccess,
                          ProcessSong processSong) throws IOException, XmlPullParserException {
        // Ok parse this bit separately.  This could have multiple images
        String image_name = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "name"));
        String image_seconds = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "seconds"));
        String image_loop = processSong.parseFromHTMLEntities(xpp.getAttributeValue(null, "loop"));
        ...
        while (!allimagesdone) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                if (xpp != null) {
                    switch (xpp.getName()) {
                        case "title":
                            image_title = new StringBuilder(processSong.parseFromHTMLEntities(xpp.nextText()));
                            break;

                        case "subtitle":
                            image_subtitle = processSong.parseFromHTMLEntities(xpp.nextText());
                            break;
                        case "notes":
                            image_notes = processSong.parseFromHTMLEntities(xpp.nextText());
                            break;
                        case "filename":
                            image_filename = processSong.parseFromHTMLEntities(xpp.nextText());
                        ...
                        case "image":
                            ...
                            break;
                        case "description":
                            String file_name = processSong.parseFromHTMLEntities(xpp.nextText());
        ...
        writeTempSlide(c.getResources().getString(R.string.image), title, c, preferences,
                storageAccess,processSong);
    }
